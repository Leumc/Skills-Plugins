...
> **铁律 5 · 异常先怀疑"环境"，不要蛮干**
> 子代理说"点了没反应 / 页面没变"时，**先排查环境原因，再重试**：
> - 跨 App 跳转的页面可能被启动到**主屏** → 用 `cmd activity display move-stack` 搬回虚拟屏（见约束 11）
> - 虚拟屏**看不到软键盘**（键盘在主屏）→ 提交用 `input keyevent 66`（见约束 12）
> - 滑动无效（画面 MD5 不变）→ 起点可能落在**底部浮层**上（见约束 13）
> - 关键点击被"多点一步" → 限制 `max_steps`（见约束 14）
...
| 命中缓存后还标"未验证/沿用旧缓存" | **已存储 = 已验证**，直接信任使用（见约束 10） |
| 跨 App 跳转后子代理说"没反应"就重试 | 先查是不是**页面跑到主屏**了（见约束 11） |
| 虚拟屏里找不到键盘就一直找 | 键盘在**主屏**，用 `input keyevent 66` 提交（见约束 12） |
| 滑动起点落在底部浮层上 | 起止点都在**页面中间可滚动区**（见约束 13） |
...
## 五之二、通用经验补充（跨场景适用，踩坑总结）

> 以下都是**与具体 App 无关**的通用规律，来自多次实测。

### 10. 已存储 = 已验证（不要自我打折）

- 数据库里已存的界面/元素/坐标，**一律视为已验证可用**，命中即信任、直接用。
- ❌ **禁止**给已存坐标标注"未验证 / 未经可靠验证 / 沿用旧缓存"之类而**降级处理**。
- 只有当**实际点击失败**时，才 `fail --id` 记录并重新定位、更新坐标。
- 主 Agent 不能因为"本次没亲自跑到"就怀疑库里的坐标。
...
**搬回虚拟屏**（唯一可用的命令）：
```bash
cmd activity display move-stack <TASK_ID> <虚拟屏display号>
# 例：cmd activity display move-stack 9948 4
# 验证：dumpsys activity activities | grep -A6 'Display #4' → 应显示 ... d4
```

> ⚠️ `am move-task` / `am task move` / `cmd activity task remove` **都不存在**（unknown command）。
> - `TASK_ID`：从上一步 `grep` 行的 `tNNNN` 取。
> - `DISPLAY_ID`：虚拟屏的**显示号**（如 `Display #4` → `4`），**不是 long id**；每次会话可能不同。
> - 退出该页面：`input -d <虚拟屏display号> keyevent 4`（返回键，不会误发给任何人）。

### 12. 虚拟屏里没有软键盘 → 提交统一用 shell 回车

**现象**：点了搜索框、输入框已聚焦，但**虚拟屏截图里看不到键盘**。
**原因**：输入法窗口 `mDisplayId=0`，**键盘渲染在主屏**，不在虚拟屏。

**正确做法**：
1. 子代理点击搜索框聚焦（坐标由主 Agent 量出）；
2. 子代理 `do(action="Type", text="关键词")` 输入；
3. **shell 提交**：`input keyevent 66`（回车），然后 sleep 2~3 秒再截图。

> ⚠️ 不要用子代理的 `do(action="Key", key="Enter")`——不可靠（可能被当成收起键盘/换行）。
> ⚠️ 输入框里灰色文字多为**占位提示**（≠已输入内容），直接 Type 即可。
> ⚠️ **点"搜索历史" ≠ 关键词搜索**，不要用历史替代输入。
> 💡 `input keyevent 111`(ESC) 可收起键盘。

### 13. 滑动三条铁律

1. **起止点必须落在页面中间的可滚动区**：
   - ❌ 起点落在**底部固定浮层/购买记录浮层/底部导航栏**（如 y>2400）→ 滑动**完全无效**（画面 MD5 不变）。
   - ✅ 正确：`start=[500,600], end=[500,200]`（起止都在内容区）。
   - 判断是否真滑动了：**对比前后截图 MD5**，相同 = 根本没滑动。
2. **目标被遮挡就再滑一点**，不要停在被遮挡状态纠结 —— 继续滑一次让它完整露出，再定位/点击。
3. **滑动 ≠ 目的**：
   - 滑动只是达成目标（找到/点击/看到某内容）的**手段**，不是任务本身。
   - 目标达成即停；还差一点就继续滑。**不能停留在"我滑过了"，也不能为滑而滑。**

### 14. 子代理可能"擅自多点一步" → 关键点击限制 max_steps

**现象**：让子代理点一次「马上抢」[744,974]，它执行 Tap → Wait → **又自己 Tap 了一次同一坐标**。
第 2 次点击时页面已变，落在新页面的同位置按钮上，**导致流程跳步**（规格被顺手点掉，直接进提交页）。

**对策**：
- 关键/敏感点击把 `max_steps` 设得**很小**（如 3），intent 里明确"**只点一次，不要重复**"。
- 点击后**再次截图**确认落在哪一步，别默认"它只点了一次"。
...
- **子代理没有"画框"动作**：其动作集为 Tap/Swipe/Note/Call_API/Launch/Back/Home/Wait，
  无法在屏幕上绘制标注。
  → "画框校验"**只能由主 Agent 在已落盘的截图上程序合成**
  （`draw_by_ratio.py`），再把框中心换算成比例坐标交给子代理。

### 16. 子代理截图会落盘（可外部读取）

每次子代理执行时"看到"的截图会保存到 `/sdcard/Download/Operit/cleanOnExit/`，
文件名形如 `<时间戳>.png`。可用 `ls -lt` 取最新一张，用于**事后核对子代理到底看到了什么**。
