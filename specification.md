...
> **这是公开仓库。严禁提交任何个人 / 本机私有数据。**

必须排除：
- **场景数据库及其等价内容**：具体界面名、元素名、**实测坐标**、操作链路明细
- 个人身份信息：学校 / 单位名、姓名、账号、联系方式
- 个人生活数据：订单号、金额、物流、聊天内容、课表、成绩、购物记录
- 凭据：token、私钥、密码
- 设备标识：IMEI / 序列号

写示例时：
- ✅ 用**占位符**：`<包名>`、`<界面名>`、`<操作名>`、`[x,y]`、`<通知正文>`
- ✅ 通用 App 名（微信 / 美团等）可作示例，但**不得附带实测坐标或个人内容**
- ❌ **禁止**把 `data/scenes.db` 的内容以任何形式（表格 / 列表 / 正文）抄进 `.md` 文件

> ⚠️ `.gitignore` 只能拦住**文件本身**；把数据库内容**手打进 Markdown 等于绕过防护**。
> 提交前自查：`grep -rnE '学校|订单|¥|[0-9]{3},[0-9]{3}' <你的目录>`，
> 确认无非占位符的坐标与个人信息。
